export function getPremiumTemplate(data) {
  const siteUrl = "https://store.primadev.id";
  const logoUrl = `${siteUrl}/primadev_light.png`;
  const licenseKey = encodeURIComponent(data.key || data.licenseKey || data.targetLicenseKey || data.transactionId || data.orderId || '');
  const invoiceUrl = `https://store.primadev.id/api/invoice?id=${licenseKey}`;
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" style="max-width: 600px; background-color: #070d17; border: 1px solid rgba(255, 255, 255, 0.12); border-radius: 16px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 32px 28px;">
              ...
              <div style="text-align: center; margin-top: 32px;">
                <a href="${invoiceUrl}" target="_blank" style="display: inline-block; background-color: #036EFD !important; color: #ffffff !important; font-weight: 800; padding: 13px 32px; border-radius: 8px; text-decoration: none; font-size: 14px;">
                  Lihat INVOICE
                </a>
          ...
          <tr>
            <td style="padding: 24px 28px; text-align: center; font-size: 12px; color: #64748b !important; border-top: 1px solid rgba(255, 255, 255, 0.08);">
              ...
              <p style="margin: 0 0 10px; color: #94a3b8 !important;">Wangon, Kecamatan Wangon, Kabupaten Banyumas, Jawa Tengah 53176</p>
